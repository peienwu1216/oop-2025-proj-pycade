...
class AIControllerBase:
    ...
    def change_state(self, new_state):
        if self.current_state != new_state: #
            ...
            if new_state not in ["TACTICAL_RETREAT_AND_WAIT", "CLOSE_QUARTERS_COMBAT", "ENGAGING_PLAYER", "ASSESSING_OBSTACLE", "MOVING_TO_BOMB_OBSTACLE"]:
                self.chosen_bombing_spot_coords = None
                self.chosen_retreat_spot_coords = None
            if new_state == "PLANNING_PATH" or new_state == "PLANNING_ROAM":
                 self.astar_planned_path = [] #
                 self.astar_path_current_segment_index = 0 #
        ...
        if self.ai_player.action_timer <= 0: #
            if self.current_movement_sub_path: #
                ...
                if sub_path_finished: #
                    self.last_decision_time = pygame.time.get_ticks() - self.ai_decision_interval # 確保下個tick立即決策 #
            else:
    ...
    def handle_roaming_state(self, ai_current_tile):
        ai_log(f"Base: In ROAMING at {ai_current_tile}. Default: go IDLE.")
        self.change_state("IDLE")
    ...
    def handle_evading_danger_state(self, ai_current_tile): #
        ...
        if not self.current_movement_sub_path: 
            ...
            min_options = getattr(self, 'min_retreat_options_for_evasion', 3)
    ...
    def astar_find_path(self, start_coords, target_coords): #
        ...
        while open_set: #
            ...
            # 檢查取出的節點是否仍然是 node_data 中的最新版本
            # 這是為了處理同一個 TileNode 被多次以不同成本加入 heap 的情況
            ...
            for neighbor_template in self._get_node_neighbors(current_node, for_astar_planning=True): #
                ...
                if neighbor_coords not in node_data or tentative_g_cost < node_data[neighbor_coords].g_cost:
                    actual_neighbor_node = node_data.get(neighbor_coords)
                    if actual_neighbor_node is None: 
                    ...
                    node_data[neighbor_coords] = actual_neighbor_node # 更新或加入字典
    ...
    def can_place_bomb_and_retreat(self, bomb_placement_coords): #
        ...
        retreat_search_depth = getattr(self, 'retreat_search_depth', 7)
        min_options = getattr(self, 'min_retreat_options', 1) 
    ...
    def find_safe_tiles_nearby_for_retreat(self, from_coords, bomb_coords_as_danger_source, bomb_range_of_danger_source, max_depth=6, min_options_needed=1): #
        ...
        q = deque([(from_coords, [from_coords], 0)]) # (coords, path_list, depth) #
        visited = {from_coords} #
            ...
            if depth < max_depth: #
                ...
                for dx, dy in shuffled_directions: #
                    ...
                    if next_coords not in visited: #
                        ...
                        if node and node.is_empty_for_direct_movement(): #
                            if not self.is_tile_dangerous(next_coords[0], next_coords[1], 0.05): # 路徑上的格子幾乎不能有任何危險 #
                                visited.add(next_coords) #
        ...
        return [spot['coords'] for spot in potential_safe_spots] #
    ...
    def execute_next_move_on_sub_path(self, ai_current_tile): #
        ...
        if self.current_movement_sub_path_index + 1 >= len(self.current_movement_sub_path):  #
            ai_log(f"    Sub-path logic error: trying to get next step beyond end. Path: {self.current_movement_sub_path}. Clearing.") #
            self.current_movement_sub_path = [] #
            self.current_movement_sub_path_index = 0 #
            return True #
