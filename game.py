...
class Game:
    def __init__(self, screen, clock, ai_archetype="original"):
        ...
        Args:
            ...
            ai_archetype (str): The keyword for the selected AI opponent.
        """
        self.screen = screen
        self.clock = clock
        self.running = True
        self.restart_game = False # <<< 新增：用於標記是否要回到主選單
        ...
        try:
            ...
            self.ai_status_font = pygame.font.SysFont("arial", 24)
        
        self.setup_initial_state()

    def setup_initial_state(self):
        """Sets up or resets the game to its initial state."""
        ...
        # AI Controller Instantiation based on menu selection
        ...
        print(f"[Game Setup] Initializing {ai_controller_class.__name__}.")
        ...
        if hasattr(self.ai_controller_p2, 'reset_state') and callable(getattr(self.ai_controller_p2, 'reset_state')):
            ...
            print(f"[Game Setup] AI Controller {self.ai_controller_p2.__class__.__name__} has no callable reset_state method.")
    ...
    def run(self):
        ...
        Main game loop.
        Returns:
            bool: True if the game should restart (return to menu), False otherwise.
        """
        while self.running:
            ...
            self.update()
            self.draw()
        return self.restart_game # <<< 修改：返回重啟標誌
    ...
    def events(self):
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                self.running = False
                self.restart_game = False # 確保正常關閉時不會重啟
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE:
                    self.running = False
                    self.restart_game = False # 確保 ESC 退出時不會重啟
    ...
    def update(self):
        ...
        if self.game_state == "PLAYING":
            ...
            # Destructible Walls vs. Explosions
            ...
            # Player vs. Items (使用 collide_rect 進行碰撞檢測更為通用)
            for player in list(self.players_group):
                if player.is_alive:
    ...
    def draw_hud(self):
        ...
        if self.player2_ai and self.hud_font:
            ...
            if self.ai_controller_p2 and self.ai_status_font:
                ...
                hud_texts.append(f"AI ({ai_name}): {state}")

        line_height = 22
        start_y_offset = 10
        for i, text_line in enumerate(reversed(hud_texts)):
            font_to_use = self.hud_font
            if text_line.startswith("AI (") and self.ai_status_font:
